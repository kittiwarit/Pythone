...
for i in range(num):
    ...
    for j in range(num):
        if hlt[i] > hlt[j]:
            ...
            hlt[i],name[j],score[j],temes[j] = a,b,c,d
    timeis = time.localtime()
    ...
    time_()
    print('-'*77)
    print('{0:-<6}{1:-<6}{2:<8}{3:-<17}{4:-<12}{5:-<15}{6}'.format('No.','PTS','TIME','COMPITITOR#name','HIT FACTOR','STATE POINTS','STAT PERCENT'))
    print('-'*77)
    for i in range(num):
